...
[ニフティクラウド SDK for Ruby](http://cloud.nifty.com/api/sdk/)が提供するコマンドを直接叩くことと比較してのメリットは以下の通りです。
...
Nifty CloudのコントロールパネルからREST APIのAccessKeyとSecretAccessKeyを[取得してください](http://cloud.nifty.com/api/rest/)。

`knife.rb`に以下のように取得したAccessKey/SecretAccessKeyと[ニフティクラウドAPIのエンドポイントURL](http://cloud.nifty.com/api/rest/endpoint.htm)を記述します。
    ...
    knife[:nifty_cloud_endpoint_url] = "https://west-1.cp.cloud.nifty.com/api/"

`knife.rb`がSCMに登録されていてAccessKey/SecretAccessKeyをknife.rbに記述したくない場合には、環境変数に登録することも可能です。
...
各サブコマンド実行時に以下のように<tt>-A</tt> (or <tt>--nifty-cloud-access-key</tt>) <tt>-K</tt> (or <tt>--nifty-cloud-secret-key</tt>)オプションとして指定することも可能です。
    ...
    knife niftycloud server create -r 'role[webserver]' -IM 21 -f small -A “Your Nifty Cloud Access Key ID“ -K "Your Nifty Cloud Secret Access Key"


`knife.rb`には以下のオプションを設定することも可能です。

* image
* nifty_cloud_ssh_key_id
* distro
* template_file
...
コマンドのオプションを確認するには<tt>--help</tt>付けてコマンドを実行してください。
...
bootstrapはデフォルトでは[ubuntu10.04-gems](https://github.com/opscode/chef/blob/master/chef/lib/chef/knife/bootstrap/ubuntu10.04-gems.erb)テンプレートを利用して実行します。
...
設定されているNifty Cloudアカウント/リージョンに登録されているのサーバ一覧を表示します。
